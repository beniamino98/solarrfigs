#' @rdname fig_traj_emp
#' @examples
#' fig_traj_emp(Amsterdam$model$scenarios$P, pVaR = 0.05, nsim = 100, nyear = 2021, alpha = 1)
#' fig_traj_emp(Oslo$model$scenarios$P, pVaR = 0.05, nyear = 2006:2023)
#' fig_traj_emp(Berlino$model$scenarios$P, pVaR = 0.05, nyear = 2010:2015)
#' fig_traj_emp(Bologna$model$scenarios$P, pVaR = 0.05, nyear = NULL, alpha = 0.5)

fig_traj_emp <- function(sim, nyear = NULL, nsim = 50, pVaR = 0.05, alpha = 0.3){
  ...
  df_PaR <- df_PaR_sim %>%
    dplyr::right_join(df_emp, by = c("Month", "Day"))
  ...
  df_plot_emp %>%
    ...
    geom_line(aes(n, GHI, group = Year), alpha = alpha, size = 0.2, color = "orange")+
    geom_point(aes(n, GHI, group = Year), alpha = alpha, size = 1.3, color = "black")+
    geom_point(aes(n, GHI, group = Year), alpha = alpha, size = 0.8, color = "orange")+
    # pVaR (simulated)
    # geom_point(aes(n, q_var_sim, group = seed), color = col_pVaR, size = 0.9)+
    # pVaR (empiric)
    geom_line(aes(n, q_var_emp, group = Year), color = col_pVaR, size = 0.5, linetype = "solid")+
    # Expected value (empiric)
    geom_line(aes(n, e_GHI_emp, group = Year), color = col_expected_value_sim, size = 0.5)+
    # Expected value (simulated)
    # geom_line(aes(n, e_GHI_sim, group = Year), color = col_expected_value_sim, size = 0.5)+
    # Seasonal mean
    geom_line(aes(n, GHI_bar, group = Year), color = "black", size = 0.5)+
    # Clearsky radiation
    geom_line(aes(n, Ct, group = Year), color = "blue", size = 1, linetype = "solid")+
